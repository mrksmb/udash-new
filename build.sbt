...
inThisBuild(Seq(
  ...
  organization := "com.smartdroplets",
  scalacOptions ++= Seq(
    "-feature",
    "-deprecation",
    "-unchecked",
...
val commonSettings = noPublishSettings ++ Seq(
  moduleName := "udash-new-" + moduleName.value,
...
lazy val frontend = project.in(file("frontend"))
  ...
  .settings(
    ...
    Compile / mainClass := Some("com.smartdroplets.frontend.JSLauncher"),
    ...
    compileCss := Def.taskDyn {
      ...
      dir.mkdirs()
      (backend / Compile / runMain).toTask(s" com.smartdroplets.backend.css.CssRenderer $path false")
    }.value,
...
lazy val backend = project.in(file("backend"))
  ...
  .settings(
    ...
    Compile / mainClass := Some("com.smartdroplets.backend.Launcher"),
  )
